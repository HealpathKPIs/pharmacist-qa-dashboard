          ...
          <section className="space-y-6">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div className="space-y-2">
                <p className="text-sm text-emerald-300">Audit trail</p>
                <h1 className="text-3xl font-semibold tracking-normal text-white">
                  Upload History
                </h1>
                <p className="max-w-2xl text-sm leading-6 text-zinc-400">
                  Review previous workbook imports and their insertion results.
                </p>
              </div>
            ...
            <Card className="animate-soft-in border-white/10 bg-white/[0.04] shadow-none">
              <CardHeader>
                ...
                <CardDescription>
                  Latest 50 upload batches from Supabase.
                </CardDescription>
              </CardHeader>
              <CardContent>
                {uploads.length === 0 ? (
                  <div className="flex flex-col items-center justify-center rounded-md border border-dashed border-white/10 px-6 py-12 text-center">
                    ...
                    <p className="mt-2 max-w-md text-sm leading-6 text-zinc-400">
                      Imported workbooks will appear here with row counts, status,
                      skipped rows, and failed rows.
                    </p>
                  </div>
                ) : (
                    ...
                    <Table>
                      <TableHeader>
                        <TableRow>
                          ...
                          <TableHead className="text-right">Inserted rows</TableHead>
                      ...
                      <TableBody>
                        {uploads.map((upload) => (
                          <TableRow key={upload.id}>
                            <TableCell className="min-w-56 font-medium text-zinc-200">
                              <span className="line-clamp-2">{upload.fileName}</span>
                            </TableCell>
                            <TableCell className="min-w-44 text-zinc-400">
                            ...
                            <TableCell className="text-right font-mono text-zinc-400">
                              {formatInteger(upload.insertedRows)}
                            </TableCell>
                            <TableCell className="text-right font-mono text-zinc-400">
                              {formatInteger(upload.failedRows)}
                            </TableCell>
                            <TableCell className="text-right font-mono text-zinc-400">
                              {formatInteger(upload.skippedRows)}
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
